...
Checked Review scope: 65 in high, 44 in outside width, 14 in deep, five shelves, spots off. The existing Alcove translates that 44 in outside width to a 42.5 in interior component span. Against promoted System and the exact tested Store source, Store selected the parent material from the finished component programs instead of accepting parent lengths from Alcove. Pine returned material 230.88 + hardware 18 + modeled machine service 132.43 = budgetary Q 381.31. Poplar returned budgetary Q 504.43. Changing pine depth to 11 in returned Q 254.15. These are observed Store-model outputs from the preserved Review Alcove, not independent validation of the machine timing model, and are not fixture prices to embed in the application.
