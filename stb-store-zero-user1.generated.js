...
Source: GeorgePlattDemo/scan-to-build-store@01f9c5580cea262bd898a9f2c1ac2cd89d02845f
...
function millPassesForDepth(totalDepthIn) {
...
function envelopeCheck(item, req = {}) {
  const reasons = [];
  const unresolved = [];
...
function spotEconomicsAssessment() {
  return {
...
function indexMin(keptLengthIn) {
  return TOOLING.accelMin + Math.abs(keptLengthIn) / TOOLING.rapidInPerMin;
}

function millLongMin(profileLengthIn = 0, passes = 1) {
...
function millEndMin(count = 0) {
  return count * MILL.endFeatureMin;
}

function cycleOneStick({
  keptLengthIn,
  widthIn,
  holes = 0,
  spots = 0,
...
function findOffering(catalog, storeSku) {
  return catalog.offerings.find((o) => o.storeSku === storeSku) || null;
}

function extendLine(catalog, storeSku, qty) {
...
function estimateJob(catalog, { title, classId, pieces, hardwareSku = null }) {
...
function estimateBoardSequence(catalog, {
  title,
...
function estimateBoardPlan(catalog, {
  title,
  classId = "user_defined_board",
  plan,
  spotCycles = 0
...
function estimatePineAlcove(catalog, shelfCount = 5) {
...
function estimateCut001(catalog) {
  return estimateJob(catalog, {
...
function estimatePicnicLegSquare(catalog) {
  return estimateJob(catalog, {
...
function estimatePicnicLegTapered(catalog, { millEnds = 1 } = {}) {
  return estimateJob(catalog, {
...
const BOARD_SEQUENCE_POLICY = Object.freeze({
...
const BOARD_SEQUENCE_RULES = Object.freeze({
  kerfIn: 0.125,
...
function resolveBoardMaterial(catalog, demand = {}) {
...
function estimateResolvedBoardPlan(catalog, materialResolution, { title, classId = "user_defined_board", spotCycles = 0 } = {}) {
...
{ estimateJob, estimatePineAlcove };
