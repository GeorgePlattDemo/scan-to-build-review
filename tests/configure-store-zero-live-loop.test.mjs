...
const catalog = read('stb-store-zero-configure-catalog.js');
...
assert.match(startOwn, /stb-store-zero-configure-catalog\.js\?v=53bf4555/);
assert.match(startOwn, /stb-store-handoff-contract\.js\?v=06a44d81/);
...
assert.match(catalog, /PINNED_STORE_ZERO_CATALOG_SLICE/);
assert.match(contract, /stb-store-zero-configure-catalog\.js/);
...
vm.runInNewContext(catalog, sandbox, { filename: 'stb-store-zero-configure-catalog.js' });
...
assert.match(shell, /stb-store-zero-configure-catalog\.js\?v=53bf4555/);
assert.match(shell, /stb-store-handoff-contract\.js\?v=06a44d81/);
