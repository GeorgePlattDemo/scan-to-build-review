...
test('actual User 1 journey is demand-driven from finished members through Store-selected parent and record', async ({ page }) => {
  ...
  expect(retained.operationPlan.finishedPart.lengthIn,'FAULT_TARGET_STOCK_LENGTH_OVERWRITES_FINISHED_GEOMETRY').toBe(16);
  ...
  await expect(project.locator('#stb-bench-remain-label')).toContainText('39⅝ in remains');
